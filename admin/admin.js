...
function setAdminLocked(isLocked) {
  ...
  if (adminShell) {
    adminShell.hidden = Boolean(isLocked);
    adminShell.classList.toggle("is-locked", Boolean(isLocked));
  }
}
...
init().catch((error) => {
  console.error("Yönetim paneli başlatılamadı:", error);
  setAdminLoading(false);
  setAdminLocked(true);
  const message = $("#loginMessage");
  if (message) message.textContent = "Panel başlatılamadı. Lütfen sayfayı yenile.";
});
